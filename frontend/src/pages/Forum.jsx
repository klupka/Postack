...
const Forum = ({
    ...
    const dislikePost = (postId, event) => {
        if (userData == "") {
            ...
            axios
                ...
                .then((res) => {
                    console.log(
                        "Post disliked. Re-rendering post information",
                        res
                    );
                    getForumPosts(currentSortOption);
                })
    ...
    const likePost = (postId, event) => {
        if (userData == "") {
            ...
            axios
                ...
                .then((res) => {
                    console.log(
                        "Post liked. Re-rendering post information",
                        res
                    );
                    getForumPosts(currentSortOption);
                })
    ...
    const handleClickOutsidePostOptions = (event) => {
        const clickedElement = event.target.closest("button"); // Find closest button element
    ...
    // 1. no forum posts, return appropriate message
    // 2. there are forum posts, return posts
    // 2.5. there are forum posts and is searching, filter posts by search value
    const displayPosts = () => {
        if (forumPosts.length <= 0 && retrievedForumPosts) {
            return (
                <div className="flex flex-col items-center justify-center gap-5 mt-10 text-text">
                    <div className="text-base text-center mx-5">
                        It looks like there are currently no posts to show.
                    </div>
                    <div className="text-base text-center mx-5">
        ...
        } else if (forumPosts.length > 0 && retrievedForumPosts) {
            if (searchValue != "") {
                const searchString = searchValue.toLowerCase();

                const searchResults = forumPosts.filter(
                    (post) =>
                ...
                if (searchResults.length > 0) {
                    return (
                        <div className="flex flex-col gap-2 mt-2">
                            {searchResults.map((forumPost, index) => {
                                let repliesCount = 0;
    ...
    else if (retrievedForumPosts != null && userData) {
        return (
            <>
                ...
                <div className="text-wrap animate-slideDown border-border w-full lg:max-w-[800px] lg:mx-auto sm:w-[calc(100%-60px)] sm:ml-[60px]">
                    <div className="pt-2 flex justify-between items-center">
                        <div className="px-4 py-2 font-bold text-2xl">Home</div>
                        <div className="" dir="rtl">
                            <button
                            ...
                            >
                                ...
                                <button
                                    onClick={() => {
                                        setCurrentSortOption("Old");
                                        handleShowHideSort();
                                    }}
                                >
                                    <div
                                        className={`h-12 flex justify-end items-center text-text gap-3 px-5 hover:bg-border ${
                                            currentSortOption === "Old"
                                                ? "bg-primary hover:bg-primary"
                                                : ""
                                        }`}
                                    >
                                ...
                                <button
                                    onClick={() => {
                                        setCurrentSortOption("Bottom");
                                        handleShowHideSort();
                                    }}
                                >
                                    <div
                                        className={`h-12 flex justify-end items-center text-text gap-3 px-5 hover:bg-border ${
                                            currentSortOption === "Bottom"
                                                ? "bg-primary hover:bg-primary"
                                                : ""
                                        }`}
                                    >
